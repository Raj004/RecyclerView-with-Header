...
class HeaderAdapter(internal var data: ArrayList<Data>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == TYPE_ITEM) {
            ...
            val itemView =
                LayoutInflater.from(parent.context).inflate(R.layout.card_new, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is VHItem) {
            ...
            if (position.equals(1)) {
                holder.itemView.setPadding(0, 50, 0, 60);
    ...
    internal inner class VHHeader(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        ...
        init {
            ...
            main_header.setOnClickListener {
                Toast.makeText(
                    itemView.context,
                    "Postion Header" + Integer.toString(position), Toast.LENGTH_LONG
                ).show()
            }
        }
    }
